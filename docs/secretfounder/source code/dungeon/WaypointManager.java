...
public class WaypointManager {
	...
	public void onWorldRender(RenderWorldLastEvent event) {
		...
		if (SecretFounder.waypointsJson.get(roomName) != null) {
			...
			for (int i = 0; i < arraySize; i++) {
				...
				String mode = secretsObject.get("category").getAsString();
				Color color;
	...
	public void onClickSecret(PlayerInteractEvent event) {
		...
		Block block = event.world.getBlockState(event.pos).getBlock(); if (block != Blocks.chest && block != Blocks.skull && block != Blocks.lever) return;
		String roomName = RoomManager.roomName; if (roomName.equals("undefined") || SecretFounder.roomsJson.get(roomName) == null || secretsList == null) return;
		if (SecretFounder.waypointsJson.get(roomName) != null) {
			JsonArray secretsArray = SecretFounder.waypointsJson.get(roomName).getAsJsonArray();
			int arraySize = secretsArray.size();
			for (int i = 0; i < arraySize; i++) {
		...
		if (event.packet instanceof S0DPacketCollectItem) {
			...
			if (entity instanceof EntityItem) {
				...
				if (name.contains("Decoy") || name.contains("Defuse Kit") || name.contains("Dungeon Chest Key")
						|| name.contains("Healing VIII") || name.contains("Inflatable Jerry")
						|| name.contains("Spirit Leap") || name.contains("Training Weights") || name.contains("Trap")
